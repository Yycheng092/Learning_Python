...
"""
在除法當中若想要同時取得商數和餘數，可以使用 divmod 函數，divmod(9,5)，所得到的結果為一個 (1,4) 的 tuple
在乘法中具有 運算子優先順序
括號(串列、集合、字典) ──► 次方──► 正數、負數、位元 not(~) ──► 乘除餘數 ──► 加減法 ──► 位元左移或右移 ──► & (位元and) ──► | (位元or) ──► 
布林判斷的 not 、 and 、 or ──► if...else
"""
 
"""
底數 
代表 二進制 (base 2) 的 0b 或 0B
代表 八進制 (base 8) 的 0o 或 0O
代表 十六進制 (base 10) 的 0x 或 0X
>>>  10  = 10
>>> 0b10 = 2
>>> 0o10 = 8 
>>> 0x10 = 16
我們也可以用任何一種底數將整束轉換成字串
>>> value = 65
>>> bin(value) #'0b1000001'
>>> oct(value) #'0o101'
>>> hex(value) #'0x41'
也可以透過 chr() 函式將整數轉換成對應的單字字串
>>> chr(65)  #'A'
反向操作
>>> ord('A') #65
